...
<?php
	...
	if (!empty($identificacion) && $identificacion!='' && !empty($nit) && $nit!=''){	
		...
		$query_participante = "SELECT ID_PARTICIPANTE FROM PARTICIPANTES WHERE ID_PAIS = " .$paisId. " AND IDENTIFICACION = '" .$identificacion . "' LIMIT 1";
		$participante = mysqli_query($connElrecqcgWeb, $query_participante) or die(mysql_error());												 												 
		...
		if (empty($idParticipante) || $idParticipante==''){
			...
			if (empty($error) || $error==''){
				...
				$tienda = mysqli_query($connElrecqcgWeb, $query_tienda) or die(mysql_error());												 												 
